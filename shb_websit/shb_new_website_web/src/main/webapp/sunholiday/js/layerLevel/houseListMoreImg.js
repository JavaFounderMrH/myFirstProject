var sriRid;
$(function(){
	sriRid = getQueryString("sriRid");
	//console.log(sriRid);
	$.ajax({
		type: "GET",
		url: "/roomInfoController/queryRoomPictureListByRoomId",
		dataType: 'json',
		data:{
			"sriRid": sriRid
		},
		success: function(data){
			if(data.messcode == "001"){
				if(data.roomPictureList.length != 0){
					var data = data.roomPictureList;
					$('#picBox .cf').empty();
					$('#listBox .cf').empty();
					var dataStr = '';
					for(var i in data){
						dataStr += '<li> <a><img src="'+data[i].picSrc+'"></a> </li>';
					}
					$('#picBox .cf').append(dataStr);
					$('#listBox .cf').append(dataStr);
